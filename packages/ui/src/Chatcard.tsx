...
interface MessageData {
    ...
    imageCaption?: string;
    type: string;
    time: string;  // For display purposes (formatted time string)
    timestamp: Date; // Optional ISO string for database operations
...
export default function ChatCard({ person, setMessageclicked }: { person: any, setMessageclicked: (clicked: boolean) => void }) {
    ...
    // ✅ FIX 1: Correctly handle session data with useEffect
    useEffect(() => {
        if (session?.user) {
    ...
    if (!socket) return;
     async function fetchChats(){
        ...
        setLoading(true);
        const chats = await fetch(`/api/chat?personNumber=${recipient.number}`).then(res => res.json());
    ...
    }, [recipient.number, socket]);

    // Effect to scroll to the bottom when new messages are added
    useEffect(() => {
        chatEndRef.current?.scrollIntoView({ behavior: 'smooth' });
    }, [newmsg]); // Trigger on any message change
    ...
    const handleSendMessage = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Add a guard to ensure user data is loaded before sending
        ...
        // ✅ FIX 2: Emit the ENTIRE message object, including the id
        ...
        <div className="absolute inset-0 flex z-1000 flex-col h-full bg-white overflow-hidden">
            ...
            <header className="flex items-center p-4 border-b border-gray-200 bg-gray-50 z-10">
                <button className="p-2 rounded-full hover:bg-gray-200 transition mr-3">
                    <ArrowLeft className="h-6 w-6 text-gray-600" onClick={() => setMessageclicked(false)} />
                </button>
                <img src={recipient.imageUrl} alt={recipient.name} className="h-10 w-10 rounded-full object-cover" />
                <div className="ml-3">
                    <h2 className="text-lg font-semibold text-gray-900">{recipient.name}</h2>
                    <p className={`text-sm ${recipient.online ? 'text-green-500' : 'text-gray-400'}`}>
                        ...
                        {messages.map((msg) => {
                            ...
                            return (
                                <div key={msg.id} className={`flex items-end gap-2 ${isSentByCurrentUser ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-xs md:max-w-md px-4 py-3 rounded-2xl ${isSentByCurrentUser ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white text-gray-800 shadow-sm rounded-bl-none'}`}>
                                        <p className="text-sm break-words">{msg.message}</p>
                                        <div className="flex items-center justify-end gap-1 mt-1">
                                            <p className={`text-xs ${isSentByCurrentUser ? 'text-indigo-100' : 'text-gray-400'}`}>
                                                {msg.time}
                                            </p>
                                            {isSentByCurrentUser && <MessageStatus status={msg.status} />}
                                        </div>
                                    </div>
                                </div>
                            );
            ...
            <footer className="p-4 bg-white border-t border-gray-200">
                <form onSubmit={handleSendMessage} className="flex items-center space-x-3">
                    <input
                        ...
                        autoComplete="off"
                    />
                    <button type="submit" className="bg-indigo-600 text-white p-3 rounded-full hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-transform duration-150 active:scale-95 disabled:bg-indigo-300" disabled={!newMessage.trim()}>
